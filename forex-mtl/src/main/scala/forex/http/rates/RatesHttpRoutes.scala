    ...
    case GET -> Root :? FromQueryParam(from) +& ToQueryParam(to) =>
      try {
        ...
        response.flatMap {
          ...
          case Left(value) => BadRequest(errors.Error.RateLookupFailed(ErrorCodes.internalError, "Rate Lookup Failed due to " + value.getMessage()).asJson)
        }
      } catch {
        case exception: Exception =>
          logger.error(exception)("Exception Occured in Forex Service.")
          BadRequest(errors.Error.RateLookupFailed(ErrorCodes.internalError, "Rate Lookup Failed due to " + exception.getMessage()).asJson)
      }
  }
